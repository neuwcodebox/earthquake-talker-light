...
2. 국내 지진 신속/상세정보
   ...
   - phase 3의 영향 지역은 발생 시각 기반의 공식 진도 상세 페이지에서 최고 진도 시·도를 우선 조회한다. 상세 페이지를 읽지 못한 경우에만 PEWS 바이너리의 영향 지역 비트를 사용한다.
   ...
   - 분석을 위해 감지된 이벤트의 원본 `.b` 파일과 수신에 성공한 위치(`.le`/`.li`), 격자(`.e`/`.i`) 파일을 `OUTPUT_DIR/pews-captures/{지진ID}`에 저장한다. 가장 최근 20개 이벤트만 보관한다.
